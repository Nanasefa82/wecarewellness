...
CREATE POLICY "Public can view active doctor profiles" ON doctor_profiles
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE profiles.id = doctor_profiles.user_id 
...
CREATE POLICY "Public can view available slots from active doctors" ON availability_slots
  FOR SELECT USING (
    is_available = true AND
    EXISTS (
      SELECT 1 FROM profiles 
      WHERE profiles.id = availability_slots.doctor_id 
      AND profiles.is_active = true
    )
  );
...
CREATE POLICY "Doctors can view appointments for their slots" ON appointments
  FOR SELECT USING (
    EXISTS (
      ...
      AND (
        availability_slots.doctor_id = auth.uid() OR
        EXISTS (
          SELECT 1 FROM profiles 
          WHERE profiles.id = auth.uid() 
          AND profiles.role = 'admin'
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Availability slot overlaps with existing slot for this doctor';
  END IF;
...
-- Trigger to check for overlaps before inserting or updating availability slots
...
-- Note: Doctor role assignment trigger is handled in the auth profiles migration